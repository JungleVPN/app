...
import { IconBrandTelegram, IconCheck, IconCopy, IconMail } from '@tabler/icons-react';
...
export function useSupportOptions() {
  ...
  return options.filter((option): option is SupportOption => option !== null);
}

export function SupportPopover({ trigger }: SupportPopoverProps) {
  const { t } = useTranslation();
